...
head(rownames(s1))
# [1] "ENSG00000268020.3"  "ENSG00000240361.1"  "ENSG00000186092.4"  "ENSG00000248527.1"  "ENSG00000228794.8" 
# [6] "ENSG00000188157.14"
...
have.gene_name<-!is.na(genes_trans[str_remove(rownames(s1),'.[0-9]+$'),on='gene_id']$hgnc_symbol)
sum(have.gene_name) #34027/36600 have gene names

s1<-s1[have.gene_name,]

mat<-s1[['RNA']]$counts
rownames(mat)<-genes_trans[str_remove(rownames(mat),'.[0-9]+$'),on='gene_id']$hgnc_symbol
